...
class Wall {
  constructor(wrapper = throwNewError`wrapper`, options = defaultOptions) {
    ...
    this.sections = this.wrapper.children.length
      ? toArray(this.wrapper.children)
      : throwNewError`sections`;
    ...
    this.easeFunction =
      typeof this.options.easeFunction === "string"
        ? easing[this.options.easeFunction]
        : this.options.easeFunction;
  ...
  _handleKeyDown(e) {
    switch (e.keyCode) {
      case 34:
      case 40:
        if (scrollTouchBottom(this.currentSection)) this.nextSection();
        break;
      case 33:
      case 38:
        if (scrollTouchTop(this.currentSection)) this.prevSection();
        break;
      case 37:
  ...
  _handleWheelEvent(e) {
    const delta = Math.max(
      -this.options.scrollThreshold,
      Math.min(this.options.scrollThreshold, e.wheelDelta || -e.detail)
    );

    console.log(-e.deltaY);

    if (
      scrollTouchBottom(this.currentSection) &&
      delta === -this.options.scrollThreshold
    ) {
      this.nextSection();
    }
    if (
      scrollTouchTop(this.currentSection) &&
      delta === this.options.scrollThreshold
    ) {
      this.prevSection();
    }
  ...
  _setupSlides() {
    this.currentSlides = toArray(
      this.currentSection.querySelectorAll(`[${SLIDE}]`)
    );
    ...
    this.sections.forEach((section) => {
      ...
      if (slides.length) {
        ...
        slides
          .reverse()
          .forEach((slide, index) => (slide.style.zIndex = index + 1));
  ...
  _lazyload(currentScreen) {
    ...
    images.forEach((image) =>
      image.setAttribute("src", image.getAttribute(IMAGE_ORIGIN))
    );
  }

  _queue(screenList) {
    screenList.reverse().forEach((section, index) => {
  ...
  _animateScreen(currentScreen, screenList) {
    ...
    this._updateCurrentScreenPosition(delta)._renderSectionPosition(
      currentScreen,
      this.currentScreenPosition
    );

    const shouldStop =
      (this.currentScreenPosition > 99.9 && !this.isToBack) ||
      (this.currentScreenPosition < 0.1 && this.isToBack);
    ...
    if (this.isAnimating) {
      return (this.requestId = rAF(
        this._animateScreen.bind(this, currentScreen, screenList)
      ));
    }
  }
  ...
  _updateCurrentScreenPosition(delta) {
    const currentDuration =
      this.screenType === SCREEN_SECTION
        ? +this.currentSection.getAttribute(ANIMATE_DURATION)
        : +this.currentSlide.getAttribute(ANIMATE_DURATION);
    const duration = currentDuration || this.options.sectionAnimateDuration;
    const target = this.isToBack ? 0 : 100;

    this.currentScreenPosition = this.easeFunction(
      delta,
      this.currentScreenPosition,
      target - this.currentScreenPosition,
      duration
    );
    return this;
  }
  ...
  _renderSectionPosition(screen, pos) {
    switch (this.screenType) {
      case SCREEN_SECTION:
        screen.style[
          transformProp
        ] = `translate(0, -${pos}%) ${this.translateZ}`;
        break;
      case SCREEN_SLIDE:
        screen.style[
          transformProp
        ] = `translate(-${pos}%, 0) ${this.translateZ}`;
        break;
    }
  }
    ...
    if (this.navElements && this.navElements.length) {
      ...
      this.navElements.forEach((navElement) => {
        ...
        navItems.forEach((item) =>
          removeClass(item, sectionNavItemActiveClass)
        );

        const currentNav = navItems.find(
          (item) =>
            item.getAttribute(SECTION_NAV_INDEX) ===
            this._getCurrentSectionIndex()
        );
  ...
  prevSection() {
    if (!this.options.loopToBottom && this._getCurrentSectionIndex() == 1)
      return;
    ...
    if (!this.isAnimating) {
      ...
      this._refreshAnimateStatus(true)._animateScreen(
        this.currentSection,
        this.sections
      );
    }
  ...
  nextSection() {
    if (
      !this.options.loopToTop &&
      this._getCurrentSectionIndex() == this.sections.length
    )
      return;
    ...
    if (!this.isAnimating) {
      ...
      this._refreshAnimateStatus(false)._animateScreen(
        this.currentSection,
        this.sections
      );
    }
  ...
  goToSection(index) {
    ...
    if (!this.isAnimating) {
      ...
      const targetSection = this.sections.find(
        (section) => section.getAttribute(SECTION_INDEX) == index
      );
      ...
      this.screenType = SCREEN_SECTION;
      this._refreshAnimateStatus(this.isToBack)._animateScreen(
        this.currentSection,
        this.sections
      );
    }
